...
def find_nccl_library() -> str:
    ...
    """
    so_file = os.environ.get("NRL_NCCL_SO_PATH")

    # manually load the nccl library
    if so_file:
        logging.info(
            "Found nccl from environment variable NRL_NCCL_SO_PATH=%s",
            so_file)
    else:
        if torch.version.cuda is not None:
            so_file = "libnccl.so.2"
        else:
            raise ValueError("NeMo-RL only supports CUDA")
        logging.info("Found nccl from library %s", so_file)
    return so_file
