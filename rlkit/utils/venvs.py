...
def create_local_venv(
    ...
    #
    # You can override this location by setting the RLKIT_VENV_DIR environment variable

    RLKIT_VENV_DIR = os.path.normpath(
        os.environ.get("RLKIT_VENV_DIR", DEFAULT_VENV_DIR)
    )
    logger.info(f"RLKIT_VENV_DIR is set to {RLKIT_VENV_DIR}.")

    # Create the venv directory if it doesn't exist
    os.makedirs(RLKIT_VENV_DIR, exist_ok=True)

    # Full path to the virtual environment
    venv_path = os.path.join(RLKIT_VENV_DIR, venv_name)
...
def _env_builder(
    ...
    venv_path = Path(RLKIT_VENV_DIR) / venv_name
...
def create_local_venv_on_each_node(py_executable: str, venv_name: str):
    ...
    force_rebuild = os.environ.get("RLKIT_FORCE_REBUILD_VENVS", "false").lower() == "true"
