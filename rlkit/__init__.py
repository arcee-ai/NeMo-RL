...
from rlkit.package_info import (
    __contact_emails__,
    __contact_names__,
    __description__,
    __download_url__,
...
def _patch_nsight_file():
    ...
    as soon as rlkit is imported, the patch is applied and the source of the nsight.py module
    is up to date before the nsight.py is actually needed.
    """
    # Only apply patch if user intends to use nsys profiling

    # Don't rely on rlkit.utils.nsys.RLKIT_NSYS_WORKER_PATTERNS since rlkit may not be available
    # on the node that imports rlkit.
    if not os.environ.get("RLKIT_NSYS_WORKER_PATTERNS"):
        return
