...
from rlkit.models.custom.state_dict_adapter import StateDictAdapter


class AFMoEStateDictAdapter(StateDictAdapter):
    ...
    def stream_to_hf(self, state_dict: dict[str, Any]):
        """Generator that yields (hf_key, hf_tensor) pairs one at a time.
        
        This streaming approach avoids OOM by not holding the entire converted
        state dict in memory. For grouped MoE expert weights, it yields each
        expert tensor individually and immediately, allowing it to be freed
        before the next one is materialized.
        
        Yields:
            Tuple[str, torch.Tensor]: (hf_key, hf_tensor) pairs
        """
        ...
        for key, value in state_dict.items():
            ...
            if m is not None:
                ...
                for expert_idx in range(value.shape[0]):
                    if which == "w1":
                        ...
                        hf_key = f"model.layers.{layer_idx}.mlp.experts.{expert_idx}.down_proj.weight"
                    # Index to get expert tensor, yield it, then it can be freed
                    expert_tensor = value[expert_idx]
                    yield hf_key, expert_tensor
                    del expert_tensor  # Hint to free memory
                continue
            ...
            if m is not None:
                layer_idx = m.group(1)
                yield f"model.layers.{layer_idx}.mlp.router.gate.weight", value
                continue
            ...
            if m is not None:
                layer_idx = m.group(1)
                yield f"model.layers.{layer_idx}.mlp.expert_bias", value
                continue
