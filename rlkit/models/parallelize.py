"""Generic parallelization for torchtitan models. Based on llama4 and afmoe parallelization."""
...
def apply_ddp(
    ...
    """Apply data parallelism (via DDP) to the model."""
    if enable_compile:
...
def parallelize_model(
    ...
    if dp_replicate > 1 or dp_shard > 1 or ep_size > 1:
...
def apply_moe_ep_tp(
    ...
    """Apply EP and TP to all MoE layers."""
    for transformer_block in model.layers.values():
        if not is_moe_enabled(transformer_block):
            continue
