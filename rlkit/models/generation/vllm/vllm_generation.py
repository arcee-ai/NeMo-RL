...
from rlkit.distributed.batched_data_dict import BatchedDataDict, SlicedDataDict
...
from rlkit.models.generation.vllm.config import VllmConfig
...
class VllmGeneration(GenerationInterface):
    def __init__(
        ...
        if self.cfg["vllm_cfg"]["async_engine"]:
            worker_cls = "rlkit.models.generation.vllm.vllm_worker_async.VllmAsyncGenerationWorker"
        else:
            worker_cls = (
                "rlkit.models.generation.vllm.vllm_worker.VllmGenerationWorker"
            )
    ...
    async def _async_generate_base(
        ...
        timeout_seconds = float(
            os.environ.get("RLKIT_VLLM_ASYNC_TIMEOUT_SECONDS", "600")
        )  # Default 10 minutes
        ...
        while not finished:
            try:
                ...
                print(
                    f"For longer sequences, increase the timeout by setting: export RLKIT_VLLM_ASYNC_TIMEOUT_SECONDS={int(timeout_seconds * 2)}"
                )
                # Cancel the task
                ...
                raise RuntimeError(
                    ...
                    f"For longer sequences, increase timeout by setting: export RLKIT_VLLM_ASYNC_TIMEOUT_SECONDS={int(timeout_seconds * 2)}"
                )
