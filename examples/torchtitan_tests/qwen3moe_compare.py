...
for key in model_tt.state_dict().keys():
    assert key in state_dict_tt.keys(), f"model_tt has key {key} but state_dict_tt does not"
for key in state_dict_tt.keys():
    assert key in model_tt.state_dict().keys(), f"state_dict_tt has key {key} but model_tt does not"
