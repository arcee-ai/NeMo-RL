...
class TestMaybeGpuProfileStep:
    ...
    def test_no_environment_variables_set(self):
        ...
        with (
            patch("rlkit.utils.nsys.RLKIT_NSYS_WORKER_PATTERNS", ""),
            patch("rlkit.utils.nsys.RLKIT_NSYS_PROFILE_STEP_RANGE", ""),
        ):
    ...
    def test_only_worker_patterns_set_raises_assertion_error(self):
        """Test that setting only RLKIT_NSYS_WORKER_PATTERNS raises AssertionError."""
        policy = MockPolicy()

        with (
            patch("rlkit.utils.nsys.RLKIT_NSYS_WORKER_PATTERNS", "worker*"),
            patch("rlkit.utils.nsys.RLKIT_NSYS_PROFILE_STEP_RANGE", ""),
        ):
            with pytest.raises(
                AssertionError,
                match="Either both RLKIT_NSYS_WORKER_PATTERNS and RLKIT_NSYS_PROFILE_STEP_RANGE must be set",
            ):
                maybe_gpu_profile_step(policy, 5)

    def test_only_step_range_set_raises_assertion_error(self):
        """Test that setting only RLKIT_NSYS_PROFILE_STEP_RANGE raises AssertionError."""
        policy = MockPolicy()

        with (
            patch("rlkit.utils.nsys.RLKIT_NSYS_WORKER_PATTERNS", ""),
            patch("rlkit.utils.nsys.RLKIT_NSYS_PROFILE_STEP_RANGE", "1:10"),
        ):
            with pytest.raises(
                AssertionError,
                match="Either both RLKIT_NSYS_WORKER_PATTERNS and RLKIT_NSYS_PROFILE_STEP_RANGE must be set",
    ...
    def test_invalid_step_range_format_non_integer_start(self):
        ...
        with (
            patch("rlkit.utils.nsys.RLKIT_NSYS_WORKER_PATTERNS", "worker*"),
            patch("rlkit.utils.nsys.RLKIT_NSYS_PROFILE_STEP_RANGE", "abc:10"),
        ):
            with pytest.raises(
                ValueError, match="Error parsing RLKIT_NSYS_PROFILE_STEP_RANGE"
            ):
                maybe_gpu_profile_step(policy, 5)
    ...
    def test_invalid_step_range_format_non_integer_stop(self):
        ...
        with (
            patch("rlkit.utils.nsys.RLKIT_NSYS_WORKER_PATTERNS", "worker*"),
            patch("rlkit.utils.nsys.RLKIT_NSYS_PROFILE_STEP_RANGE", "1:xyz"),
        ):
            with pytest.raises(
                ValueError, match="Error parsing RLKIT_NSYS_PROFILE_STEP_RANGE"
            ):
    ...
    def test_start_step_greater_than_stop_step(self):
        ...
        with (
            patch("rlkit.utils.nsys.RLKIT_NSYS_WORKER_PATTERNS", "worker*"),
            patch("rlkit.utils.nsys.RLKIT_NSYS_PROFILE_STEP_RANGE", "10:5"),
        ):
    ...
    def test_start_step_equal_to_stop_step(self):
        ...
        with (
            patch("rlkit.utils.nsys.RLKIT_NSYS_WORKER_PATTERNS", "worker*"),
            patch("rlkit.utils.nsys.RLKIT_NSYS_PROFILE_STEP_RANGE", "5:5"),
        ):
            with pytest.raises(AssertionError, match="must be a non-empty range"):
                maybe_gpu_profile_step(policy, 5)
    ...
    def test_start_step_less_than_one(self):
        ...
        with (
            patch("rlkit.utils.nsys.RLKIT_NSYS_WORKER_PATTERNS", "worker*"),
            patch("rlkit.utils.nsys.RLKIT_NSYS_PROFILE_STEP_RANGE", "0:10"),
        ):
    ...
    def test_negative_start_step(self):
        ...
        with (
            patch("rlkit.utils.nsys.RLKIT_NSYS_WORKER_PATTERNS", "worker*"),
            patch("rlkit.utils.nsys.RLKIT_NSYS_PROFILE_STEP_RANGE", "-5:10"),
        ):
            with pytest.raises(AssertionError, match="must be >= 1"):
                maybe_gpu_profile_step(policy, 5)

    @patch("rlkit.utils.nsys.rich.print")
    @patch("rlkit.utils.nsys.atexit.register")
    def test_start_profiling_in_range(self, mock_atexit_register, mock_rich_print):
        ...
        # Should set the flag
        assert hasattr(policy, "__RLKIT_PROFILE_STARTED")
        assert getattr(policy, "__RLKIT_PROFILE_STARTED") is True
    ...
    def test_start_profiling_at_start_boundary(
        ...
        with (
            patch("rlkit.utils.nsys.RLKIT_NSYS_WORKER_PATTERNS", "worker*"),
            patch("rlkit.utils.nsys.RLKIT_NSYS_PROFILE_STEP_RANGE", "3:8"),
        ):
    ...
    def test_continue_profiling_when_already_started(self, mock_rich_print):
        ...
        policy = MockPolicy()
        setattr(policy, "__RLKIT_PROFILE_STARTED", True)
    ...
    def test_stop_profiling_after_range(self, mock_rich_print):
        ...
        assert getattr(policy, "__RLKIT_PROFILE_STARTED") is False
    ...
    def test_no_action_when_step_before_range(self):
        ...
        policy.start_gpu_profiling.assert_not_called()
        policy.stop_gpu_profiling.assert_not_called()
        assert not hasattr(policy, "__RLKIT_PROFILE_STARTED")
    ...
    def test_single_step_range(self, mock_atexit_register):
        ...
        with (
            patch("rlkit.utils.nsys.rich.print"),
            patch("rlkit.utils.nsys.RLKIT_NSYS_WORKER_PATTERNS", "worker*"),
            patch("rlkit.utils.nsys.RLKIT_NSYS_PROFILE_STEP_RANGE", "5:6"),
        ):
            maybe_gpu_profile_step(policy, 5)
        assert getattr(policy, "__RLKIT_PROFILE_STARTED") is True
        policy.start_gpu_profiling.assert_called_once()
    ...
    def test_profiling_sequence_full_lifecycle(self, mock_atexit_register):
        ...
        with (
            patch("rlkit.utils.nsys.rich.print"),
            patch("rlkit.utils.nsys.RLKIT_NSYS_WORKER_PATTERNS", "worker*"),
            patch("rlkit.utils.nsys.RLKIT_NSYS_PROFILE_STEP_RANGE", "3:7"),
        ):
    ...
    def test_large_step_numbers(self, mock_atexit_register):
        ...
        with (
            patch("rlkit.utils.nsys.rich.print"),
            patch("rlkit.utils.nsys.RLKIT_NSYS_WORKER_PATTERNS", "worker*"),
            patch("rlkit.utils.nsys.RLKIT_NSYS_PROFILE_STEP_RANGE", "1000000:1000002"),
        ):
...
class TestEdgeCases:
    ...
    def test_step_zero_handling(self):
        ...
        with (
            patch("rlkit.utils.nsys.RLKIT_NSYS_WORKER_PATTERNS", "worker*"),
            patch("rlkit.utils.nsys.RLKIT_NSYS_PROFILE_STEP_RANGE", "1:5"),
        ):
